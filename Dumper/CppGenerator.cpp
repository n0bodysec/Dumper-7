...
// {}
// Size -> 0x{:04X} (FullSize[0x{:04X}] - InheritedSize[0x{:04X}])
{}{}{} {}{}{}{}
{{
)", Struct.GetFullName()
...
R"({
	...
	return nullptr;
		...
		PredefinedFunction {
			...
			.ReturnType = "std::vector<UEType*>", .NameWithParams = "FindObjects(const std::string& FullName, EClassCastFlags RequiredType = EClassCastFlags::None)",
			.NameWithParamsWithoutDefaults = "FindObjects(const std::string& FullName, EClassCastFlags RequiredType)", .Body =
R"({
	...
	return ret;
		...
		PredefinedFunction {
			.CustomComment = "",
			.CustomTemplateText = "template<typename UEType = UObject>",
			.ReturnType = "std::vector<UEType*>", .NameWithParams = "FindObjectsFast(const std::string& Name, EClassCastFlags RequiredType = EClassCastFlags::None)",
			.NameWithParamsWithoutDefaults = "FindObjectsFast(const std::string& Name, EClassCastFlags RequiredType)", .Body =
R"({
	std::vector<UEType*> ret;
	for (int i = 0; i < GObjects->Num(); ++i)
	{
	...
	return ret;
		...
		PredefinedFunction {
			.CustomComment = "",
			.CustomTemplateText = "template<typename UEType = UObject>",
			.ReturnType = "UEType*", .NameWithParams = "FindObject(EClassCastFlags RequiredType = EClassCastFlags::None)", .Body =
R"({
	...
	return nullptr;
		...
		PredefinedFunction {
			.CustomComment = "",
			.CustomTemplateText = "template<typename UEType = UObject>",
			.ReturnType = "std::vector<UEType*>", .NameWithParams = "FindObjects(EClassCastFlags RequiredType = EClassCastFlags::None)", .Body =
R"({
	auto v = UEType::StaticClass();
	std::vector<UEType*> ret;
	for (int i = 0; i < GObjects->Num(); ++i)
...
R"({
	*this /= Magnitude();
	...
	/* FTransform */
	PredefinedElements& FTransformPredefs = PredefinedMembers[ObjectArray::FindObjectFast<UEStruct>("Transform").GetIndex()];
	...
	FTransformPredefs.Functions =
	{
		...
		PredefinedFunction{
			.CustomComment = "",
			.ReturnType = "FMatrix", .NameWithParams = "ToMatrixWithScale()", .Body =
R"({
	FMatrix OutMatrix;
	...
	/* FMatrix */
	PredefinedElements& FMatrixPredefs = PredefinedMembers[ObjectArray::FindObjectFast<UEStruct>("Matrix").GetIndex()];

	FMatrixPredefs.Functions =
	{
		/* const operators */
		PredefinedFunction {
			.CustomComment = "",
			.ReturnType = "FMatrix", .NameWithParams = "operator*(const FMatrix& Other)", .Body =
R"({
	FMatrix ret;
	...
	/* FLinearColor */
	PredefinedElements& FLinearColorPredefs = PredefinedMembers[ObjectArray::FindObjectFast<UEStruct>("LinearColor").GetIndex()];

	FLinearColorPredefs.Functions =
	{
		/* constructors */
		PredefinedFunction {
			.CustomComment = "CONSTRUCTOR", .NameWithParams = "FLinearColor()", .Body =
R"({
	R = 0;
	G = 0;
	B = 0;
	A = 0;
		...
		PredefinedFunction {
			.CustomComment = "CONSTRUCTOR", .NameWithParams = "FLinearColor(float r, float g, float b, float a)", .Body =
R"({
	R = r;
	G = g;
	B = b;
	...
	SortFunctions(FMatrixPredefs.Functions);
	SortFunctions(FLinearColorPredefs.Functions);
}
